...
No more `npm start` / `react-scripts` — the project moved off Create React App to Vite. Only `src/App.test.js` exists (a render smoke test using Vitest + `@testing-library/react` + jsdom), so `npm test` verifies almost nothing — don't treat a green run as coverage.
